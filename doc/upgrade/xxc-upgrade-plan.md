...
本计划已拆分为 4 个独立的 `/goal`，每个是一段可直接粘贴执行的命令。本文档是它们的共享参考（调查数据、必改清单、风险表）。
...
**依赖链**：Goal 1 → Goal 2 → Goal 3 → Goal 4，必须按顺序执行。每个 goal 的 Done-when 都依赖前一个 goal 的产物（尤其 Goal 1 的截图基线和 mock server）。
