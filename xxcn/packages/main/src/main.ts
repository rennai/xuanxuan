...
if (process.env.DEBUG && process.env.DEBUG !== 'production') {
...
const installExtensions = async () => {
  ...
  console.log(
    '>> 正在安装 Electron 调试扩展...首次启动可能需要花费几分钟时间，如果长时间没有出现主界面窗口，请尝试退出此命令行任务，然后执行 "npm run start-hot-fast" 来代替 "npm run start-hot" 命令。'
  )
