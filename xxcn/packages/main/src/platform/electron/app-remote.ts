...
if (typeof DEBUG === 'undefined') {
  global.DEBUG =
    process.env.NODE_ENV === 'debug' || process.env.NODE_ENV === 'development'
} else {
  global.DEBUG = DEBUG
}
...
class AppRemote {
        ...
        result
          .then((x) => {
            try {
              ...
              console.error(
                '\n>> ERROR: Cannot send remote result to BrowserWindow.',
                err
              )
            }
            return x
    ...
    ipcMain.on(EVENT.remote_on, (e, eventId, event) => {
      events.on(event, (...args) => {
        try {
          ...
          if (SHOW_LOG) {
            console.error(
              `\n>> Remote event '${event}' has be force removed, because window is closed.`,
              e
            )
          }
        }
          ...
          submenu: DEBUG
            ? [
                ...
                },
                {
                  ...
                  click: () => {
                    this.currentFocusWindow.setFullScreen(
                      !this.currentFocusWindow.isFullScreen()
                    )
                  },
                },
                {
        ...
        },
        {
          ...
          submenu: [
            ...
            },
            {
              ...
              click() {
                shell.openExternal(
                  'https://github.com/easysoft/xuanxuan/issues'
                )
              },
            },
          ],
  ...
  createTrayIcon(windowName = 'main') {
    ...
    // 创建一个通知栏图标
    const tray = new Tray(
      `${this.entryPath}/${this.appConfig.media['image.path']}tray-icon-16.png`
    )
    ...
    this._trayIcons = [
      ...
      nativeImage.createFromPath(
        `${this.entryPath}/${this.appConfig.media['image.path']}tray-icon-transparent.png`
      ),
    ]
  }
    ...
    if (options.debug && DEBUG) {
      ...
      browserWindow.webContents.on('crashed', () => {
        const messageBoxOptions = {
          ...
          message:
            'The renderer process has been crashed, you can reload or close it.',
  ...
  get currentFocusWindow() {
    const focusedWindowName = Object.keys(this.windows).find((winName) =>
      this.windows[winName].isFocused()
    )
    return focusedWindowName
      ? this.windows[focusedWindowName]
      : this.mainWindow || this.windows[Object.keys(this.windows)[0]]
  }
      ...
      if (flash) {
        if (!trayData.flashTask) {
          trayData.flashTask = setInterval(() => {
            if (trayData.tray) {
              trayData.tray.setImage(
                this._trayIcons[trayData.iconCounter++ % 2]
              )
            }
          }, 400)
        }
