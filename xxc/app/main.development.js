...
// [upgrade] 供 preload.js 计算 appRoot 使用
...
// [upgrade] 移除 electron-debug（不兼容新 Electron），改用内置 openDevTools（由 app-remote 在 DEBUG 时调用）
...
const installExtensions = async () => {
    ...
    if (process.env.NODE_ENV === 'development') {
        // [upgrade] electron-devtools-installer 未安装时优雅跳过（React DevTools 为可选）
        let installer;
        try {
            installer = require('electron-devtools-installer'); // eslint-disable-line global-require
        } catch (_) { // eslint-disable-line no-empty
            console.log('>> electron-devtools-installer 未安装，跳过扩展安装。');
            return;
        }
        const extensions = [
